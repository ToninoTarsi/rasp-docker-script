...
yum -y install netcdf-fortran libpng15 iproute-tc tcp_wrappers-libs sendmail procmail psmisc procps-ng mailx findutils ImageMagick perl-CPAN ncl netcdf libpng libjpeg-turbo which perl-Tk.x86_64 patch perl-Env.noarch glibc.i686 libpng12.i686
...
curl -SL http://www.drjack.info/RASP/DOWNLOAD/rasp_scripts.tar.gz | tar xCz $BASEDIR
curl -SL http://www.drjack.info/RASP/DOWNLOAD/rasp_ncl.tar.gz | tar xCz $BASEDIR
curl -SL http://www.drjack.info/RASP/DOWNLOAD/drjack_utils.tar.gz | tar xCz $BASEDIR
curl -SL http://www.drjack.info/RASP/DOWNLOAD/wrf_execs.tar.gz | tar xCz $BASEDIR
...
ln -sf grib_prep.exe.gfs $BASEDIR/WRF/wrfsi/bin/grib_prep.exe
...
echo export PATH+=:\$BASEDIR/bin >> .bashrc 
echo export LD_LIBRARY_PATH=\$BASEDIR/UTIL/PGI >> .bashrc
